    ...
    <?php foreach ($_SESSION['posts'] as $post): ?>
        <a href="detail.php?id=<?=$post["id"]?>">
            <article class="flex flex-col flex-1 bg-white shadow-lg rounded-lg p-10 m-5 max-w-100 transition-all duration-300 hover:shadow-xl hover:scale-105">
                <h2 class="text-4xl font-semibold"><?=htmlspecialchars($post["title"])?></h2>
                <h3>By: <span class="font-semibold"> <?=htmlspecialchars($post["author"]["userName"])?></span></h3>
                <p class="truncate mt-4 line-clamp-3 whitespace-break-spaces leading-4.5"><?=htmlspecialchars($post["content"])?></p>
            </article>
        </a>
    <?php endforeach;?>
</section>
